...
async function createDocument(args, user) {
  ...
  try {
    ...
    let baseName = name.replace(/\s*\(AI\)\s*$/, '').replace(/\.(pdf|txt|docx?)$/i, '');
    ...
    await new Promise((resolve, reject) => {
      ...
      doc.fontSize(10).font('Helvetica').fillColor('#666666')
        .text(`Generated by AI Assistant • ${new Date().toLocaleDateString()}`, { align: 'center' });
      doc.moveDown(1);
      ...
      for (const line of lines) {
        if (line.startsWith('# ')) {
          doc.moveDown(0.5).fontSize(16).font('Helvetica-Bold').text(line.substring(2));
          doc.fontSize(11).font('Helvetica');
        } else if (line.startsWith('## ')) {
          doc.moveDown(0.5).fontSize(14).font('Helvetica-Bold').text(line.substring(3));
          doc.fontSize(11).font('Helvetica');
        } else if (line.startsWith('### ')) {
          doc.moveDown(0.3).fontSize(12).font('Helvetica-Bold').text(line.substring(4));
          doc.fontSize(11).font('Helvetica');
      ...
      doc.fontSize(9).fillColor('#999999')
        .text('This document was generated by AI and should be reviewed before use.', { align: 'center' });
    ...
    return {
      success: true,
      message: `Created PDF document "${savedDoc.name}"${locationInfo}`,
      document: {
        id: savedDoc.id,
        name: savedDoc.name,
...
**MANDATORY: ALWAYS ADD MATTER NOTES - THIS IS CRITICAL**
After EVERY substantive action (creating documents, scheduling events, drafting emails), you MUST:
1. Call add_matter_note with the matter_id
2. Document exactly what you did
3. Include any important details (dates, amounts, names)

Example: After creating a document, immediately call:
add_matter_note({ matter_id: "xxx", content: "Created engagement letter with $5,000 retainer and hourly rate of $350. Document ready for client signature." })

NEVER skip this step. The matter notes are the audit trail of all work performed.
...
You have just completed Phase 1 of the task. Now evaluate your work and create follow-up actions.
...
**REQUIRED PHASE 2 ACTIONS - You MUST include these:**

1. **MATTER NOTE (MANDATORY)** - Add a comprehensive note to the matter summarizing:
   - What work was completed
   - Key details (dates, amounts, names)
   - Next steps or action items
   - Use add_matter_note with the matter_id

2. **TIME ENTRY (MANDATORY)** - Log billable time for the work:
   - Use log_time with appropriate hours (0.25 to 2.0 typically)
   - Include detailed description of work performed

3. **CALENDAR EVENT** - If there are any deadlines, meetings, or follow-ups:
   - Use create_calendar_event
   - Set appropriate reminders

4. **DRAFT EMAIL** - If client communication is needed:
   - Use draft_email to prepare client communication
   - Include summary of work or next steps

5. **ADDITIONAL IMPROVEMENTS** - Any other valuable additions

**RESPOND BY CALLING think_and_plan** with:
- goal: "Complete follow-up actions for: ${goal}"
- analysis: Your evaluation including what notes, time entries, and communications are needed
- steps: Array of 4-8 specific follow-up actions (MUST include matter note and time entry)

DO NOT skip the matter note or time entry. These are mandatory for every task.`;

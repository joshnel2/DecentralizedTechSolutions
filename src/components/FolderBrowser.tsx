...
import { 
  ...
  FolderPlus, Edit3, Trash2, X, Check, Upload
} from 'lucide-react'
...
export function FolderBrowser({ 
  ...
  const [_deletingFolderId, _setDeletingFolderId] = useState<string | null>(null)
  ...
  // Rename folder
  const _renameFolder = async (folderId: string, newName: string) => {
    if (!newName.trim()) return
    
    try {
      await driveApi.renameFolder(folderId, newName.trim())
      _setRenamingFolderId(null)
      _setRenameValue('')
      invalidateFolderBrowserCache()
      fetchData(true)
    } catch (err: any) {
      ...
      {contextMenuFolder && (
        ...
        >
          <button 
            ...
            onClick={() => {
              _setRenamingFolderId(contextMenuFolder.id)
              _setRenameValue(contextMenuFolder.name)

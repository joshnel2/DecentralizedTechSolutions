...
import { Bot, X, StopCircle, MessageSquare, Rocket } from 'lucide-react'
...
export function BackgroundTaskBar() {
  ...
  const _isErrorState = hasError || activeTask.status === 'error' || activeTask.status === 'failed'
  const _isCancelledState = isCancelled || activeTask.status === 'cancelled' || activeTask.status === 'cancelling'
